...
// VALIDATION
    ...
    if ($san_first === false || $firstname == '') {
        $errors['firstname'] =  "Veuillez indiquer votre prÃ©nom.";
      }else {
      $errors['firstname'] = "";
      }
...
try {
    ...
    //body content
    $body = "<p><strong>Bonjour</strong>, le " . $date . " Ã  " .$heure . " ,  " . $name ." a envoyÃ© ce mail, c est un " . $choix . " qui a comme message " . $message ."</p>";
   ...
   if ($mail->send())
    ...
    if ($handle->uploaded) {
      if ($handle->file_src_name_ext == 'png' and $handle->file_src_name_ext == 'jpg' and $handle->file_src_name_ext == 'jpeg' and $handle->file_src_name_ext == 'gif') {
